...
final class v419ContainerUIIds
{
	...
	public static function write(ByteBufferWriter $out, int $containerId) : void
	{
		if ($containerId > self::RECIPE_BOOK) {
			$containerId--;
		} elseif ($containerId === self::RECIPE_BOOK) {
			throw new InvalidArgumentException("Invalid container ID for protocol version: v419");
		}

		Byte::writeUnsigned($out, $containerId);
	}

	public static function read(ByteBufferReader $in) : int
	{
