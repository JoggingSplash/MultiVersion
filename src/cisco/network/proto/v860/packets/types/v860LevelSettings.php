...
namespace cisco\network\proto\v860\packets\types;
...
final class v860LevelSettings {
	...
	// Since 1.26, these fields are almost at the end of the buffer
	...
	static public function fromLatest(LevelSettings $levelSettings) : self     {
		$level = new self();
		foreach (get_object_vars($levelSettings) as $key => $value) {
			if(!property_exists($level, $key)) {
				continue;
			}
			$level->$key = $value;
		}
